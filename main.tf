...
module "log-bucket" {
  ...
  depends_on = [module.tf-service-account, time_sleep.wait_for_apis]
}
...
module "tf-gcs-buckets" {
  ...
  depends_on = [time_sleep.wait_for_apis, module.tf-service-account, module.log-bucket, google_kms_crypto_key.tf-kms-crypto-key]
}
...
resource "google_storage_bucket_iam_policy" "tf-gcs-buckets" {
  ...
  depends_on = [module.tf-gcs-buckets, time_sleep.wait_for_apis]
}
...
resource "google_storage_bucket_iam_policy" "log-bucket" {
  ...
  depends_on  = [module.log-bucket, time_sleep.wait_for_apis]
}
...
resource "google_storage_bucket_iam_member" "log_sink_writer" {
  ...
  depends_on = [google_logging_project_sink.logs-sink, time_sleep.wait_for_apis]
}
