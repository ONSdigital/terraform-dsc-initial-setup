...
| [time_sleep.wait_for_metrics](https://registry.terraform.io/providers/hashicorp/time/latest/docs/resources/sleep) | resource |
...
| <a name="input_environment"></a> [environment](#input\_environment) | The environment for the resources (must be one of: sandbox, dev, staging, prod, ci). | `string` | n/a | yes |
| <a name="input_force_enable_monitoring"></a> [force\_enable\_monitoring](#input\_force\_enable\_monitoring) | Set to true to enable monitoring even in non-prod/non-staging environments.<br/>Has no effect if environment is 'prod' or 'staging', as monitoring is always enabled for those. | `bool` | `false` | no |
...
| <a name="input_monitoring_notification_channel_ids"></a> [monitoring\_notification\_channel\_ids](#input\_monitoring\_notification\_channel\_ids) | List of notification channel IDs to associate with alerting policies.<br/>These can be created manually in the GCP console, or via Terraform.<br/>See: https://www.terraform.io/docs/providers/google/r/monitoring_notification_channel.html | `list(string)` | `[]` | no |
